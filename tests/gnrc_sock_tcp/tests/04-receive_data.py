...
def testfunc(child):
    ...
    child.sendline('sock_tcp_connect [{}]:{} 0 0'.format(target_addr, str(port)))
    child.expect_exact('sock_tcp_connect: returns 0')

    # Accept Data sent by the host system
    child.sendline('sock_tcp_read 1000000 ' + str(data_len))
    child.expect_exact('sock_tcp_read: received ' + str(data_len), timeout=20)

    # Close connection and verify that pktbuf is cleared
    shutdown_event.set()
    child.sendline('sock_tcp_disconnect')
    server_handle.join()
